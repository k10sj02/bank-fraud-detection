...
with st.sidebar:
    ...
    if page == "📊  Dashboard":
        ...
        threshold = st.slider(
            ...
            0.45,
            0.01,
            help="Lower = more donations flagged. Higher = only flag the most suspicious.",
        )
    ...
    if page == "📊  Dashboard":
        ...
        min_match_ui = st.slider(
            "OFAC match threshold",
            0.50,
            0.99,
            0.75,
            0.01,
...
can focus their review time where it matters most.
    ...
    steps = [
        (
            ...
            "Get a quick read on how many donations have been flagged today and whether the anomaly rate is within expected bounds.",
        ),
        (
            "2",
            "Check Model Performance",
            "Verify the model is performing well. A healthy ROC-AUC is above 0.80 and Average Precision above 0.40.",
        ),
        (
            "3",
            "Review the Triage Queue",
            "Work through Critical and High tier donations first. Each row shows the risk score and which signals triggered the flag.",
        ),
        (
            "4",
            "Use the Donation Scorer",
            "If a specific donation has been reported to you, enter its details directly to get an instant risk score and explanation.",
        ),
        (
            "5",
            "Explore the EDA tabs",
            "Use the analysis tabs to spot systemic patterns — e.g. a specific region or platform with unusually high anomaly rates.",
        ),
    ]
    ...
    tier_guide = [
        (
            ...
            RED,
            "Score 0.80–1.00",
            "Block and escalate to senior compliance officer immediately. Do not process until reviewed.",
        ),
        (
            "High",
            CORAL,
            "Score 0.60–0.79",
            "Queue for compliance review within 24 hours. Monitor donor for further activity.",
        ),
        (
            "Medium",
            GOLD,
            "Score 0.30–0.59",
            "Log and monitor. Review if the donor appears again with additional flags.",
        ),
        (
            "Low",
            TEAL,
            "Score 0.00–0.29",
            "No action required. Donation is within normal parameters.",
        ),
    ]
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    k2.metric(
        ...
        f"{meta['n_anomalous']:,}",
        f"{meta['anomaly_rate']:.1%} of total",
    )
    k3.metric("Detection Accuracy (AUC)", f"{cur_m['auc']}")
    k4.metric("Finds Real Anomalies (AP)", f"{cur_m['ap']}")
    k5.metric(
        "Late-Night Anomaly Rate", f"{meta['night_rate']:.1%}", "Midnight–5am donations"
    )
    k6.metric(
        "Via Untraceable Platforms",
        f"{meta['high_risk_n']:,}",
        "Virtual Card / QR / Chatbot",
    )
    ...
    pm1.metric(
        "Accuracy When It Flags",
        f"{cur_m['precision']:.1%}",
        help="Of all donations the model flags as anomalous, what % are actually anomalous?",
    )
    pm2.metric(
        "Anomalies It Catches",
        f"{cur_m['recall']:.1%}",
        help="Of all truly anomalous donations, what % does the model find?",
    )
    pm3.metric(
        "Balanced Score (F1)",
        f"{cur_m['f1']:.3f}",
    ...
    pm4.metric(
        ...
        f"{cur_m['auc']:.4f}",
        help="1.0 = perfect. 0.5 = no better than random. Above 0.80 is reliable.",
    )

    col_cm, col_pr = st.columns([1, 1.7])

    with col_cm:
        section_label("Confusion Matrix — what the model predicted vs reality")
...
- **{fn:,} False Negatives** — real anomalies the model missed ⚠️ *(adjust threshold to catch more)*
    ...
    with col_pr:
        section_label(
            "Precision-Recall — the trade-off between catching more vs being more accurate"
        )
        ...
        ax.axhline(
            meta["anomaly_rate"], color=RULE, lw=1.2, ls="--", label="Random baseline"
        )
...
The **threshold slider** in the sidebar moves your operating point along this curve.
    ...
    with tab1:
        ...
        bp_data = [
            eda_df[eda_df["Is_Anomalous"] == i]["Donation_Amount"] for i in [0, 1]
        ]
        bp = axes[1].boxplot(
            bp_data,
            patch_artist=True,
    ...
    with tab2:
        ...
        axes[0].bar(
            hourly.index, hourly.values * 100, color=bar_c, alpha=0.72, width=0.8
        )
        axes[0].axhline(
            avg * 100, color=GOLD, lw=1.5, ls="--", label=f"Baseline {avg:.1%}"
        )
        axes[0].set_title("Anomaly Rate by Hour of Day")
        axes[0].set_xlabel("Hour (0 = midnight, 12 = noon)")
        axes[0].set_ylabel("% of donations flagged")
        axes[0].legend(frameon=False, fontsize=8)
        axes[0].spines["bottom"].set_visible(True)
        ...
        axes[1].bar(
            daily.index, daily.values * 100, color=bar_c2, alpha=0.72, width=0.6
        )
        axes[1].axhline(
            avg * 100, color=GOLD, lw=1.5, ls="--", label=f"Baseline {avg:.1%}"
        )
    ...
    with tab3:
        ...
        ax.set_title("Anomaly Rate by Region — Top 20 Highest Risk")
        ...
        insight(
            ...
            f"Regions shown in red exceed the overall average. In a production system, "
    ...
    with tab4:
        ...
        riskiest_device, device_rate = top_cats["Device_Type"]
        insight(
    ...
    with tab5:
        ...
        ax.barh(
            imp.index[::-1], imp.values[::-1], color=clrs[::-1], alpha=0.8, height=0.65
        )
        ...
        ax.set_title("What the model relies on most to detect anomalies")
    ...
    with ofac_tab:
        ...
        if run_screening:
            ...
            if not names:
                ...
                results_out = []
                with st.spinner(
                    f"Screening {len(names)} names against sanctions lists…"
                ):
                    for name in names:
                        is_sdn = any(
                        ...
                        top_score = (
                            f"{res.hits[0].match_score:.2f}" if res.hits else "—"
                        )
                ...
                if blocked > 0:
                    ...
                    for name in names:
                        is_sdn = any(
                            sdn.lower() in name.lower() for sdn in DEMO_SDN_NAMES
                        )
                        res = (
                            inject_demo_hit(name)
                            if (demo_mode and is_sdn)
                            else screen_donor(name, min_match=min_match_ui)
                        )
                        if not res.is_blocked:
                            continue
                        for hit in res.hits:
                            programs_str = (
                                ", ".join(hit.programs) if hit.programs else "—"
                            )
                            st.markdown(
    ...
    with col_tbl:
        ...
        filtered = filtered.sort_values("Anomaly_Score", ascending=False).head(
            n_risk_rows
        )
        display_map = {
    ...
    section_label("Enter details for any donation to get an instant risk assessment")
    st.markdown(
    ...
    with st.form("score_form"):
        ...
        demo_scorer = fn2.checkbox(
            "Demo mode (SDN name injection)",
            value=True,
            help="Injects a synthetic OFAC hit if you type a known SDN name. Disable with real data.",
        )

        fc1, fc2, fc3, fc4 = st.columns(4)
        s_amount = fc1.number_input(
            "Donation Amount (INR)", 100.0, 100000.0, 5000.0, 500.0
        )
        ...
        st.markdown(
            ...
            f"Banking Partner Signals</p>",
            unsafe_allow_html=True,
        )
        bp1, bp2, bp3 = st.columns(3)
        s_cvv_pass = bp1.selectbox(
            "CVV Check",
            ["Pass ✅", "Fail ❌", "Not applicable"],
            help="Result of real-time card CVV verification from the payment gateway.",
        )
        s_device_known = bp2.selectbox(
            "Device Fingerprint",
            ["Known clean device", "First-seen device", "Known flagged device ⚠️"],
            help="Device fingerprint status from the payment gateway. Known flagged devices have been associated with prior fraud.",
        )
        s_webhook_event = bp3.selectbox(
            "Webhook Event",
            [
                "payment.authorised",
                "payment.reversed",
    ...
    if submitted:
        ...
        if ofac_result.is_blocked:
            ...
            st.success(
                f"✅ **Sanctions clear** — no matches above {min_match_ui:.0%} threshold on any loaded list."
            )
        ...
        sample["Low_Balance_Large_Gift"] = int(
            s_balance < bal_p10 and s_amount >= amt_p75
        )
        ...
        fired_rules = []
        if (
            sample["Is_Night_Donation"]
            and sample["Is_Transfer"]
            and s_amount >= amt_p75
        ):
            fired_rules.append(
                (
                    "🌙",
                    "Night-time large transfer",
                    "Large transfers between midnight and 5am are unusual for individual donors and may indicate automated activity.",
                )
            )
        if sample["High_Risk_Platform"] and s_amount >= amt_p75:
            fired_rules.append(
                (
                    "📱",
                    "High-risk platform + large amount",
                    "Virtual cards, QR codes, and chatbot payments are harder to trace. Large amounts via these channels are elevated risk.",
                )
            )
        if sample["Amt_Bal_Ratio"] > 3.5:
            fired_rules.append(
                (
                    "⚖️",
                    "Donation far exceeds account balance",
                    f"Donation is {sample['Amt_Bal_Ratio']:.1f}× the account balance — suggests a pass-through account.",
                )
            )
        if sample["Low_Balance_Large_Gift"]:
            fired_rules.append(
                (
                    ...
                    "Account balance is in the bottom 10% of donors, yet donation is in the top 25%. Classic mule account pattern.",
                )
            )
        if sample["Is_Round_Amount"] and s_amount >= amt_p75:
            fired_rules.append(
                (
                    "🔢",
                    "Large round-number amount",
                    "Round numbers above ₹74,000 are a structuring signal — money launderers often use clean figures.",
                )
            )
        if sample["Near_Threshold"]:
            fired_rules.append(
                (
                    ...
                    f"Amount is within 1% of a reporting ceiling (₹{'/'.join(str(t) for t in struct_thresh)}).",
                )
            )
        if sample["Young_Large"]:
            fired_rules.append(
                (
                    "👤",
                    "Young donor + very large amount",
                    "Donor under 25 making a very large donation — disproportionate to typical income for this age group.",
                )
            )
        if sample["Is_Withdrawal"] and s_amount >= amt_p95:
            fired_rules.append(
                (
                    ...
                    "A very large withdrawal may indicate layering — funds cycling through the charity account.",
                )
            )
        ...
        rc4.metric("Amount / Balance", f"{sample['Amt_Bal_Ratio']:.2f}×")
        ...
        (
            ...
            [
                (
                    ...
                    "A donation of ₹90,000 followed by a refund request a few days later.",
                ),
                (
                    "Structuring",
                    ...
                    "Making a donation of ₹49,800 instead of ₹50,000 to stay below a reporting ceiling.",
                ),
                (
                    "Layering",
                ...
                (
                    ...
                    "Account balance of ₹5,000 making a donation of ₹80,000.",
                ),
                (
                    "Mule Account",
                ...
                (
                    ...
                    "A donation of ₹49,950 when your reporting threshold is ₹50,000.",
                ),
                (
                    "OFAC Sanctions",
        ...
        (
            ...
            [
                ...
                (
                    ...
                    "A ₹85,000 donation via Virtual Card scores significantly higher than the same amount via bank transfer.",
                ),
                (
                    "Amt/Balance Ratio",
                    ...
                    "Ratio of 8.2 means an account with ₹10,000 balance is donating ₹82,000.",
                ),
                (
                    "Night Donation",
                ...
                (
                    ...
                    "Balance ₹8,000 (bottom 10%) + donation ₹80,000 (top 25%) = this flag fires.",
                ),
                (
                    "Near Threshold",
                    "A flag set when a donation falls within 1% below a reporting ceiling: ₹50,000, ₹75,000, ₹90,000, or ₹99,000. This narrow band is unlikely by chance and suggests deliberate structuring.",
                    "A donation of ₹49,750 falls within 1% of ₹50,000 — this flag fires.",
                ),
                (
                    "Feature Importance",
