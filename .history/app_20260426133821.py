...
def confusion_matrix_fig(cm, title):
    ...
    for i in range(2):
        for j in range(2):
            ...
            ax.add_patch(rect)
            ax.text(j + 0.5, 1 - i + 0.60, f"{cm[i][j]:,}",
                    ha="center", va="center",
                    fontsize=24, fontweight="bold",
                    color=cell_colors[i][j], fontfamily="serif")
            ax.text(j + 0.5, 1 - i + 0.28, cell_labels[i][j],
                    ha="center", va="center",
                    fontsize=7.5, color=SLATE, fontfamily="sans-serif",
                    linespacing=1.5)
...
k2.metric("Anomalous",          f"{meta['n_anomalous']:,}", f"{meta['anomaly_rate']:.1%} of total")
...
with col_pr:
    ...
    for mname, scol, clr in model_styles:
        ...
        ax.plot(r, p, color=clr, lw=lw, alpha=alf,
...
with tab2:
    ...
    axes[0].set_xlabel("Hour (0–23)"); axes[0].set_ylabel("Anomaly Rate (%)")
    axes[0].legend(frameon=False, fontsize=8)
...
with tab3:
    top = (eda_df.groupby("Region")["Is_Anomalous"]
           .agg(["mean", "count"])
...
with tab4:
    cat_cols = ["Donation_Type", "Campaign_Category", "Donation_Channel", "Device_Type", "Currency"]
    fig, axes = plt.subplots(2, 3, figsize=(15, 8.5))
    axes = axes.flatten(); fig.patch.set_facecolor(CREAM)
...
with col_sum:
    ...
    ax.barh(tier_order[::-1], [int(tc[t]) for t in tier_order[::-1]],
            color=clrs_bar[::-1], alpha=0.72, height=0.5)
    ax.set_xlabel("Count", fontsize=7); ax.tick_params(labelsize=7)
    ax.grid(True, axis="x"); ax.spines["bottom"].set_visible(True)
    plt.tight_layout(); st.pyplot(fig, use_container_width=True); plt.close()
...
with st.form("score_form"):
    ...
    s_balance     = fc2.number_input("Account Balance (INR)", 100.0, 100000.0, 20000.0, 1000.0)
    s_age         = fc3.number_input("Donor Age", 18, 90, 35)
    s_hour        = fc4.slider("Hour of Day", 0, 23, 14)

    fc5, fc6, fc7, fc8 = st.columns(4)
    s_platform    = fc5.selectbox("Donation Platform", HIGH_RISK_PLATFORMS + [
        "Debit/Credit Card", "Web Browser", "Mobile Device", "ATM", "Desktop/Laptop"])
    s_don_type    = fc6.selectbox("Transaction Type", ["Transfer", "Withdrawal", "Debit", "Credit", "Bill Payment"])
...
if submitted:
    ...
    tier     = ("Low" if score < 0.30 else
                "Medium" if score < 0.60 else
                "High" if score < 0.80 else "Critical")
