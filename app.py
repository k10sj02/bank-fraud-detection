...
def page_footer():
    ...
    st.markdown(
        f'<div style="border-top:1px solid {RULE};padding:16px 0 8px;margin-top:2rem;'
        f'text-align:center;font-size:0.72rem;color:{MUTED};font-family:Inter,sans-serif;">'
        f"DonorGuard · Donation Compliance Platform &nbsp;·&nbsp; "
        f'Built by <a href="https://github.com/k10sj02" target="_blank" '
        f'style="color:{PURPLE};text-decoration:none;">Stann-Omar Jones</a>'
        f"</div>",
        unsafe_allow_html=True,
    )
...
with st.sidebar:
    ...
    if page == "📊  Dashboard":
        ...
        _opt_default = (
            float(OPT_THRESHOLDS.get(model_choice, 0.55)) if arts is not None else 0.55
        )
        threshold = st.slider(
            "Decision threshold",
            0.10,
    ...
    st.markdown(
        ...
        "python prepare.py</code> to retrain.<br><br>"
        '<span style="color:#6a6e87;">Built by </span>'
        '<a href="https://github.com/k10sj02" target="_blank" '
        'style="color:#9b84e0;text-decoration:none;">Stann-Omar Jones</a>'
        "</div>",
        unsafe_allow_html=True,
    )
...
The **threshold slider** in the sidebar moves your operating point along this curve.
    ...
    with tab3:
        top = (
            eda_df.groupby("Donor_Country")["Is_Anomalous"]
            .agg(["mean", "count"])
    ...
    with tab6:
        if shap_vals is None:
            ...
            st.markdown(
                f'<p style="font-size:0.84rem;color:{SLATE};line-height:1.6;margin-bottom:1rem;">'
                f"Mean absolute SHAP value across 500 test donations — how much each feature "
                f"shifts the model anomaly score on average. Higher = more influential.</p>",
                unsafe_allow_html=True,
            )
            ...
            colors = [
                PURPLE if i < 5 else (SLATE if i < 10 else MUTED)
            ...
            st.markdown(
                f'<p style="font-size:0.84rem;color:{SLATE};line-height:1.6;margin-bottom:1rem;">'
                f"Mean signed SHAP value — red bars push the anomaly score up (suspicious signal), "
                f"blue bars push it down (legitimacy signal).</p>",
                unsafe_allow_html=True,
            )
            ...
            ax.set_title(
                "Mean Signed SHAP — Red = Flags Donations, Blue = Clears Donations"
            )
    ...
    with st.form("score_form"):
        ...
        s_account = d2.selectbox(
            "Donor Segment", ["Individual", "Corporate", "Major Donor"]
        )
        s_freq = d3.selectbox(
            "Donation Frequency", ["First-time", "Occasional", "Regular", "Lapsed"]
        )
        ...
        s_balance = da2.number_input(
            "Donor Lifetime Value (£)", 0.0, 20000.0, 200.0, 50.0
        )
        s_platform = da3.selectbox(
            "Donation Platform",
            [
                "Campaign Website",
                "JustGiving",
        ...
        s_don_type = da4.selectbox(
            "Donation Type", ["Bank Transfer", "Direct Debit", "Card", "Gift", "Refund"]
        )
        ...
        s_ip_velocity = ip3.number_input(
            "IP Velocity (24h)",
            min_value=1,
            max_value=50,
            value=1,
        ...
        submitted = st.form_submit_button(
            "Calculate Risk Score →", use_container_width=True
        )
    if submitted:
        ...
        sample["Donation_Amount_Zscore"] = (s_amount - amount_mean) / (
            amount_std + 1e-9
        )
        sample["Hour"] = s_hour
        ...
        sample["Is_Corporate_Night"] = int(
            s_account == "Corporate" and s_hour <= 4 and s_don_type == "Bank Transfer"
        )
        sample["Is_First_Time_Large"] = int(
            s_freq == "First-time" and s_amount >= amt_p90
        )
        ...
        sample["IP_Mismatch_Large"] = int(
            s_ip_match == "Mismatch ⚠️" and s_amount >= amt_p75
        )
        ...
        if sample["Amt_LTV_Ratio"] > 3.5:
            fired_rules.append(
                (
                    ...
                    f"Donation is {sample['Amt_LTV_Ratio']:.1f}× the donor's total lifetime giving — suggests a pass-through account.",
                )
            )
        ...
        if sample["Is_Round_Amount"] and s_amount >= amt_p75:
            fired_rules.append(
                (
                    ...
                    "Large round numbers (£500, £1,000, £5,000) are a structuring signal — money launderers often use clean figures.",
                )
            )
