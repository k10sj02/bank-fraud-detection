...
A compliance tool for detecting anomalous or potentially bad-actor donations in a humanitarian campaign context. Originally adapted from a bank transaction fraud dataset, then fully reshaped into a realistic nonprofit donation schema with domain-specific feature engineering, realistic synthetic labels, sanctions screening via Watchman, and a Streamlit compliance dashboard.
...
This system detects suspicious donation patterns for the Syria Campaign — flagging potential money laundering via charity, structuring, pass-through accounts, card testing, chargeback abuse, and sanctions evasion. It operates across two layers:
...
├── Donations_Syria_Campaign.csv                   ← generated by step 1
├── Donations_Syria_Campaign_Labeled.csv           ← generated by step 2
...
The original `Is_Fraud` labels are randomly assigned and carry no predictive signal. The bank schema has been fully remapped to a nonprofit donation domain via `reshape_dataset.py`. See [The Label Problem](#️-the-label-problem) and [Dataset Reshaping](#dataset-reshaping) for details.
